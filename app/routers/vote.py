...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_users)):
    
    post = db.query(models.Post).filter(models.Vote).filter(models.Post.id == vote.post_id).first()
    if not post:
